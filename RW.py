...
labels
 ...
 'id':1},
 {'name:'Недопустимо',
'id':2},{'name':'Сверху', 'id':3}, {'name':'Снизу', 'id':4},{'name':'Слева', 'id':5},
{'name':'Справа', 'id':6},{'name':'Груз', 'id':7}]
...
f.write('\tid:{}\n'.format(label['id']))
f.write('}\n')
!python {SCRIPTS_PATH + '/generate_tfrecord.py'} -x {IMAGE_PATH +
'/train'}
 -l
 {ANNOTATION_PATH
 +
...
!cp
{PRETRAINED_MODEL_PATH+'/ssd_mobilenet_v2_fpnlite_320x320_coco17
_tpu-8/pipeline.config'} {MODEL_PATH+'/'+CUSTOM_MODEL_NAME}
...
CONFIG_PATH
 =
MODEL_PATH+'/'+CUSTOM_MODEL_NAME+'/pipeline.config'
...
pipeline_config.train_config.fine_tune_checkpoint
 =
PRETRAINED_MODEL_PATH+'/ssd_mobilenet_v2_fpnlite_320x320_coco17_
tpu-8/checkpoint/ckpt-0'
...
MODEL_PATH,CUSTOM_MODEL_NAME,MODEL_PATH,CUSTOM_MOD
EL_NAME))
import os
...
input_tensor
 =
 tf.convert_to_tensor(np.expand_dims(image_np,
 0),
dtype=tf.float32)
